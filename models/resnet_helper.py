...
class BasicTransform(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            dilation (int): size of dilation. Not used.
        ...
        self._norm_name = get_normalization_name(norm_type)
    ...
    def forward(self, x):
        x = self.a(x)
        x = getattr(self, "a_" + self._norm_name)(x)
        # x = self.a_bn(x)
        x = self.a_relu(x)

        x = self.b(x)
        x = getattr(self, "b_" + self._norm_name)(x)
        # x = self.b_bn(x)
        return x
...
class BottleneckTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        # Branch2c
        x = self.c(x)
        x = getattr(self, "c_" + self._norm_name)(x)
        # x = self.c_bn(x)
        return x
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        if hasattr(self, "branch1"):
            x = (
                getattr(self, "branch1_" + self._norm_name)(self.branch1(x)) +
                self.branch2(x)
            )
